...
class Elasticsearch_instance():
    def __init__(self,server):
        self.es=Elasticsearch(server,http_auth=('elastic','elastic'))
...
if __name__=='__main__':
    ...
    es_instance.create_index('test1',mapping=mapping)#Add ind0ex
    data={"title":"to be or not to be" ,"year":1952,"wiki_page":'leonardo@10'}
    #es_instance.add_data(data,'test1')        #Add dato index
    query={
            "bool": {
                "must": {
                    "match_phrase": {
